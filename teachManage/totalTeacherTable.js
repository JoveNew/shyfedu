TEACHER_URL = "teachManage/totalTeacherTable.ashx";
...
function getData() {
    var submitData = "LOAD_TEACHER;" + year_code;
    ajaxPost(TEACHER_URL, submitData, getTeacher);
}
...
function getTeacher(data) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        for (var j = 0; j < 51; j++) {
            if (j == 0)
                temp += "<td id='" + i + "_" + j + "'>" + obj[i].teacher_name + "</td>\n";
            else
                temp += "<td id='" + i + "_" + j + "' class='small'>&nbsp;</td>\n";
        }
        temp += "<\tr>";
    }
    $("tbody").append(temp);
    var submitData = "LOAD_INFO;" + year_code;
    ajaxPost(TEACHER_URL, submitData, getTable);
}
...
function getTable(data) {
    ...
    for (var i = 0; i < obj.length; i++) {
        for (var j = 0; j < rows; j++) {
            if (obj[i].teacher_name == $("#" + j + "_0").text()) {
                var detail = obj[i].subject_type_name + "<br>" + obj[i].class_name + "<br>" + obj[i].room_name;
                if (detail != "") {
                    var location = 0;
