<%@ WebHandler Language="C#" Class="teachCase" %>
...
public class teachCase : IHttpHandler,IReadOnlySessionState {
    ...
    public string loadInfo(string submitData)
    {
        ...
        string sql = @"select plan_file,class_name,subject_type_name,account_name,subject_hour from data_plan t1 
                        left join data_subject t2 on t1.subject_idx = t2.subject_idx 
                        left join data_class t3 on t2.class_idx = t3.class_idx
                        left join dbo.dict_subject_type t4 on t2.subject_type = t4.subject_type
